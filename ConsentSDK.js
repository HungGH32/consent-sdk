(function () {
  const systemId = document.currentScript.getAttribute("data-system-id");
  const apiUrl = `https://your-api.com/consent-banner?systemId=${systemId}`;

  function loadBannerConfig() {
    fetch(apiUrl)
      .then((response) => response.json())
      .then((data) => renderConsentBanner(data))
      .catch((error) => console.error("Error loading consent banner:", error));
  }

  function renderConsentBanner(data) {
    // ${data.banner_text} to load config
    ...
    // Handle action for banner
    document
      .getElementById("accept-btn")
      .addEventListener("click", () => handleConsent(true, data));
    document
      .getElementById("reject-btn")
      .addEventListener("click", () => handleConsent(false, data));
  }
  // Render current

  function handleConsent(consentGiven, data) {
    const consentPayload = {
      system_id: data.system_id,
      consent_date: new Date().toISOString(),
      consent_given: consentGiven,
      consent_ip: "AUTO_FILL_BY_SERVER",
      consent_user_agent: navigator.userAgent,
    };

    fetch("https://your-api.com/save-consent", {
      method: "POST",
      ...
      .then((response) => {
        if (response.ok) {
          ...
          localStorage.setItem("userConsent", JSON.stringify(consentPayload));
        }
      })
